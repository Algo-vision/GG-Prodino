...
def format_status(status: Dict) -> str:
    ...
    lines.append(f"  Firmware:       {cfg.get('firmwareVersion', status.get('firmwareVersion', 'N/A'))}")
    lines.append(f"  Controller IP:  {cfg.get('controllerIp', status.get('controllerIp', 'N/A'))}")
    lines.append(f"  Router IP:      {status.get('routerIp', 'N/A')}")
    lines.append(f"  Whitelist:      {cfg.get('whitelistIps', status.get('whitelistIps', []))}")
    ...
    lines.append(f"  Technician Mode:{cfg.get('technicianMode', status.get('technicianMode', False))}")
    ...
    if power_connected:
        lines.append(f"  System Voltage: {status.get('systemVoltage', 0):.2f}V")
        lines.append(f"  System Current: {status.get('systemCurrent_A', 0):.3f}A")
