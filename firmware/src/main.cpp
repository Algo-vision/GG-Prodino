...
#include <utility/w5100.h>   // [HTTPWD] raw socket state for the listener watchdog
...
#include "telemetry_bench.hpp"   // [BENCH] harness
#include "secure_telemetry.hpp"  // encrypted status uplink to the dashboard server
...
void setup() {
  ...
  // Start the 50 ms real-time ISR: LED and relay timing runs from here so it
  // holds regardless of what the main loop is doing.
  startRtIsr50ms();
...
void loop() {
  ...
  // Encrypted telemetry to the dashboard server. Only the head of a send
  // blocks (~19 ms: build + encrypt + write); the reply and the socket close
  // are finished by telemetryPump() across later iterations, and the TCP
  // connection is held open so the handshake is paid once, not per send.
...
#define TELEM_INTERVAL_MS 60000UL
  ...
  handleSerialCommands();      // SET_SN / GET_SN / SET_KEY / GET_KEY_STATUS
